...
LONG FillPngImageHeaderFromBitmapInfo(_In_ const LPBITMAPINFO pbmi, _In_ BOOL bGrayscale, _In_ BOOL bInterlace, _Out_ PNG_IMAGE_HEADER* pImageHeader);
LONG CopyPaletteFromDIB(_In_ const LPBITMAPINFO pbmi, _Out_ void* pPal);
void RGB16to24(_In_ WORD rgb16, _Out_ BYTE& r, _Out_ BYTE& g, _Out_ BYTE& b);
void CopyPngPixelFromDIB(_In_ const LPBITMAPINFO pbmi, _In_ BYTE* pDib, _In_ PNG_IMAGE_HEADER* pihdr, _Out_ BYTE* pPng);
LONG CopyImageDataFromDIB(_In_ const LPBITMAPINFO pbmi, _In_ void* pvBits, _In_ PNG_IMAGE_HEADER* pihdr, _Out_ void* pidat, _In_ DWORD dwAvailSize);
...
LONG compress(_In_ void* pvIn, _In_ DWORD dwSizeIn, _Out_ void** ppvOut, _Out_ LPDWORD pdwSizeOut, _In_ int level)
{
	...
	*pdwSizeOut = 0;
	...
	unsigned char* buf_in = (unsigned char*)malloc(ZLIB_BUF_SIZE);
	if(buf_in==nullptr)
	{
		return PNG_NOT_ENOUGH_MEMORY;
	}
	unsigned char* buf_out = (unsigned char*)malloc(ZLIB_BUF_SIZE);
	if(buf_out==nullptr)
	{
		free(buf_in);
		return PNG_NOT_ENOUGH_MEMORY;
	}
	unsigned long destSize = dwSizeIn;// allocate memory for extraction destinate (final output)

	unsigned char* pdest = (unsigned char*)malloc(destSize);
	if(pdest==nullptr)
	{
		free(buf_in);
		free(buf_out);
	...
	z_stream strm;/* allocate deflate state */
	strm.zalloc = Z_NULL;
	strm.zfree = Z_NULL;
	strm.opaque = Z_NULL;
	...
	unsigned char* psrc = (unsigned char*)pvIn;
	const unsigned long srcSize = dwSizeIn;
	unsigned long bytesRead = 0;
	unsigned long bytesWrite = 0;
	int flush;
	do {/* compress until end of data */
		if(bytesRead<srcSize)
		{
			...
			{
				const unsigned long n = min(ZLIB_BUF_SIZE, srcSize-bytesRead);
	...
	if(ret==Z_STREAM_END)
	{
		if(bytesWrite<destSize)// free extra memory
		{
			destSize = bytesWrite;
			pdest = (unsigned char*)realloc(pdest, destSize);
		}
...
LONG FilterImageData(
	...
	_In_ BOOL bBest)
{
	ASSERT(pihdr!=nullptr&&pvImageData!=nullptr);

	const UINT nBpp = PNGBytesPerPixel(pihdr);
	if(nBpp==0)
	{
	...
	{
		const UINT nBytesPerLine = PNGBytesPerLine(pihdr);
	...
	else if(pihdr->ihInterlaceMethod==PNG_INTERLACE_ADAM7)
	{
		...
		{
			const UINT nPassLines = PNGPassLines(pihdr, pass);
			const UINT nPassPixels = PNGPassLinePixels(pihdr, pass);

			// don't process empty passes
			if(nPassLines>0&&nPassPixels>0)
			{
				const UINT nBytesInLine = nPassPixels*nBpp;
				const UINT nPassSize = nPassLines*(nBytesInLine+1/*1 for filter byte*/);
...
LONG FillPngImageHeaderFromBitmapInfo(_In_ const  LPBITMAPINFO pbmi, _In_ BOOL bGrayscale, _In_ BOOL bInterlace, _Out_ PNG_IMAGE_HEADER* pImageHeader)
{
	ASSERT(pbmi!=nullptr&&pImageHeader!=nullptr);
...
LONG CopyPaletteFromDIB(_In_ const LPBITMAPINFO pbmi, _Out_ void* pPal)
{
	ASSERT(pbmi!=nullptr&&pPal!=nullptr);
	ASSERT(pbmi->bmiHeader.biSize>=sizeof(BITMAPINFOHEADER));
	...
	const UINT numColors = min(256, DIBNumColors(pbmi));
...
void RGB16to24(_In_ WORD rgb16, _Out_ BYTE& r, _Out_ BYTE& g, _Out_ BYTE& b)
{
	const WORD R_Mask = 0x00007c00;
	const WORD G_Mask = 0x000003e0;
	const WORD B_Mask = 0x0000001f;
...
void CopyPngPixelFromDIB(_In_ LPBITMAPINFO pbmi, _In_ BYTE* pDib, _In_ PNG_IMAGE_HEADER* pihdr, _Out_ BYTE* pPng)
{
	if(pihdr->ihBitDepth<=8)
	{
		if(pihdr->ihColorType==PNG_COLOR_TYPE_GRAYSCALE)
		{
			...
			{
				const DIB_RGB_TRIPLE* prgbDib = (DIB_RGB_TRIPLE*)pDib;
		...
		else if(pihdr->ihColorType==PNG_COLOR_TYPE_GRAYSCALE_ALPHA)
		{
			...
			const DIB_RGBA_QUAD* prgbaDib = (DIB_RGBA_QUAD*)pDib;
...
{
	ASSERT(pbmi!=nullptr&&pvBits!=nullptr&&pihdr!=nullptr&&pidat!=nullptr);
	ASSERT(pbmi->bmiHeader.biWidth==pihdr->ihWidth);
	ASSERT(abs(pbmi->bmiHeader.biHeight)==pihdr->ihHeight);

	const DWORD dwImageSize = CalcPngImageSize(pihdr);
	...
	const UINT dibBpp = DIBBytesPerPixel(pbmi);
	const UINT pngBpp = PNGBytesPerPixel(pihdr);
	...
	const UINT dibStride = DIBBytesPerLine(pbmi);

	BYTE* pDibByte = nullptr;
	...
	else if(pihdr->ihBitDepth<8)
	{
		if(pihdr->ihInterlaceMethod==PNG_INTERLACE_SEQUENTIAL)
		{
			const UINT dibIncrement = pihdr->ihBitDepth!=2 ? 1 : 2;
			const UINT nBytesPerLine = PNGBytesPerLine(pihdr);
			if(nBytesPerLine==0)
				return PNG_BAD_FORMAT;
...
/*
 ...
 If pbytes is not nullptr, the function copeis bytes from.
 If pbytes is nullptr, the function skip bytes and only write chunk header and crc
*/
LONG MakeChunk(
...
LONG WritePngFile(
	...
	_In_ const LPBITMAPINFO pvDIBInfo,
	_In_ const  LPVOID pvDIBData,
...
{
	ASSERT(ppvPNGData!=nullptr&&pdwPNGSize!=nullptr&&pvDIBData!=nullptr);
	if(ppvPNGData==nullptr||pdwPNGSize==nullptr||pvDIBData==nullptr)
	...
	*pdwPNGSize = 0;
	...
	LONG lRet = PNG_FAIL;
	DWORD dwPngSize = 0;
	LPBYTE pPngData = nullptr;
	PNG_IMAGE_HEADER ihdr;
	void* pvPalette = nullptr;
	DWORD dwPaletteSize = 0;
	void* pvImageData = nullptr;
	DWORD dwImageSize = 0;
	void* pvCompressedImageData = nullptr;
	...
	__try
	{
		...
		lRet = FillPngImageHeaderFromBitmapInfo(pvDIBInfo, bGrayscale, bInterlace, &ihdr);
		if(lRet!=PNG_SUCCESS)
			return lRet;
		...
		if(ihdr.ihColorType==PNG_COLOR_TYPE_INDEXED)
		{
			...
			if(pvPalette==nullptr)
			{
				lRet = PNG_NOT_ENOUGH_MEMORY;
				return lRet;
			}

			lRet = CopyPaletteFromDIB(pvDIBInfo, pvPalette);
			if(lRet!=PNG_SUCCESS)
				return lRet;
		...
		if(pvImageData==nullptr)
		{
			return PNG_NOT_ENOUGH_MEMORY;
		}

		// Copy image pixels from input DIB
		lRet = CopyImageDataFromDIB(pvDIBInfo, pvDIBData, &ihdr, pvImageData, dwImageSize);
		if(lRet!=PNG_SUCCESS)
			return lRet;
		...
		// compress image data
		lRet = compress(pvImageData, dwImageSize, &pvCompressedImageData, &dwCompressedImageSize, Z_BEST_COMPRESSION);
		if(lRet!=PNG_SUCCESS)
			return lRet;
		...
		if(ihdr.ihColorType==PNG_COLOR_TYPE_INDEXED)
		{
			ASSERT(pvPalette!=nullptr&&dwPaletteSize!=0);
		...
		lRet = MakeChunk(PNG_CHUNK_TYPE_IEND, nullptr, 0, pFilePtr, dwPngSize-(pFilePtr-pPngData));
		if(lRet!=PNG_SUCCESS)
			return lRet;
		// ================================================
	...
	__finally
	{
		...
		if(pvCompressedImageData!=nullptr)
			free(pvCompressedImageData);

		if(lRet!=PNG_SUCCESS&&pPngData!=nullptr)
		{
			free(pPngData);
		}
	}
