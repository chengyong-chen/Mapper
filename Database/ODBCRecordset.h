...
class __declspec(dllexport) CDBField : public CDBVariant
{
...
public:
	...
	CDBField& operator =(bool bVal);

	CDBField& operator =(char chVal);

	CDBField& operator =(unsigned char chVal);

	CDBField& operator =(short sVal);

	CDBField& operator =(int iVal);

	CDBField& operator =(long lVal);

	CDBField& operator =(float fVal);

	CDBField& operator =(double fVal);

	CDBField& operator =(const COleDateTime& dtVal);

	CDBField& operator =(const CString& cVal);

	CDBField& operator =(LPCTSTR szVal);
...
private:
	bool m_bIsNull; //	If this field contain a nullptr value
...
/////////////////////////////////////////////////////////////////////////////
inline const CString& CDBField::GetName() const
{
	return m_cName;
}

inline CDBField::operator bool() const
{
	return AsBool();
}

inline CDBField::operator unsigned char() const
{
	return AsChar();
}

inline CDBField::operator short() const
{
	return AsShort();
}

inline CDBField::operator int() const
{
	return AsInt();
}

inline CDBField::operator long() const
{
	return AsLong();
}

inline CDBField::operator float() const
{
	return AsFloat();
}

inline CDBField::operator double() const
{
	return AsDouble();
}

inline CDBField::operator COleDateTime() const
{
	return AsDate();
}

inline CDBField::operator CString() const
{
	return AsString();
}

inline CDBField::operator CLongBinary*() const
{
	return nullptr;
}
...
class __declspec(dllexport) CODBCRecordset : public CRecordset
{
public:
	// Our constructor uses CRecordset constructor
	CODBCRecordset(CDatabase* pDatabase = nullptr);

	~CODBCRecordset() override;

	virtual BOOL Open(LPCTSTR lpszSQL, UINT nOpenType = AFX_DB_USE_DEFAULT_TYPE, DWORD dwOptions = 0);

	void Move(long nRows, WORD wFetchType = SQL_FETCH_RELATIVE) override;// throw( CDBException, CMemoryException );
	...
	int GetFieldId(LPCTSTR szName) const;

	//	Get the field name by Id
	CString GetFieldName(int nId) const;

	//	Gets a field by name
	CDBField& Field(LPCTSTR szName) const;

	//	Gets a field by number
	CDBField& Field(int nField) const;

	//	Gets a field by name
	CDBField& operator()(LPCTSTR szField) const;

	//	Gets a field by number
	CDBField& operator()(int nField) const;

	//	Get values by field names. Do conversion as appropriate.

	bool GetBool(LPCTSTR szName) const;

	unsigned char GetChar(LPCTSTR szName) const;

	short GetShort(LPCTSTR szName) const;

	int GetInt(LPCTSTR szName) const;

	long GetLong(LPCTSTR szName) const;

	float GetFloat(LPCTSTR szName) const;

	double GetDouble(LPCTSTR szName) const;

	COleDateTime GetDate(LPCTSTR szName) const;

	CString GetString(LPCTSTR szName) const;

	CLongBinary* GetBinary(LPCTSTR szName) const;

	//	Get values by field number. Do conversion as appropriate.

	bool GetBool(int nCol) const;

	unsigned char GetChar(int nCol) const;

	short GetShort(int nCol) const;

	int GetInt(int nCol) const;

	long GetLong(int nCol) const;

	float GetFloat(int nCol) const;

	double GetDouble(int nCol) const;

	COleDateTime GetDate(int nCol) const;

	CString GetString(int nCol) const;

	CLongBinary* GetBinary(int nCol) const;
...
private:
	...
	static void AllocDataBuffer(CDBVariant& varValue, CODBCFieldInfo& fi);

	static short GetCFieldType(short nSQLType);
};
...
inline
//	Gets a field by name
CDBField& CODBCRecordset::Field(LPCTSTR szName) const
{
...
inline
//	Gets a field by number
CDBField& CODBCRecordset::Field(int nField) const
{
	//	There is no allocated storage
	ASSERT(m_fields!=nullptr);
...
inline CDBField& CODBCRecordset::operator()(LPCTSTR szField) const
{
	return Field(szField);
}

inline CDBField& CODBCRecordset::operator()(int nField) const
{
	return Field(nField);
}
...
inline bool CODBCRecordset::GetBool(LPCTSTR szName) const
{
	return Field(szName).AsBool();
}

inline unsigned char CODBCRecordset::GetChar(LPCTSTR szName) const
{
	return Field(szName).AsChar();
}

inline short CODBCRecordset::GetShort(LPCTSTR szName) const
{
	return Field(szName).AsShort();
}

inline int CODBCRecordset::GetInt(LPCTSTR szName) const
{
	return Field(szName).AsInt();
}

inline long CODBCRecordset::GetLong(LPCTSTR szName) const
{
	return Field(szName).AsLong();
}

inline float CODBCRecordset::GetFloat(LPCTSTR szName) const
{
	return Field(szName).AsFloat();
}

inline double CODBCRecordset::GetDouble(LPCTSTR szName) const
{
	return Field(szName).AsDouble();
}

inline COleDateTime CODBCRecordset::GetDate(LPCTSTR szName) const
{
	return Field(szName).AsDate();
}

inline CString CODBCRecordset::GetString(LPCTSTR szName) const
{
	return Field(szName).AsString();
}

inline CLongBinary* CODBCRecordset::GetBinary(LPCTSTR szName) const
{
...
inline bool CODBCRecordset::GetBool(int nCol) const
{
	return Field(nCol).AsBool();
}

inline unsigned char CODBCRecordset::GetChar(int nCol) const
{
	return Field(nCol).AsChar();
}

inline short CODBCRecordset::GetShort(int nCol) const
{
	return Field(nCol).AsShort();
}

inline int CODBCRecordset::GetInt(int nCol) const
{
	return Field(nCol).AsInt();
}

inline long CODBCRecordset::GetLong(int nCol) const
{
	return Field(nCol).AsLong();
}

inline float CODBCRecordset::GetFloat(int nCol) const
{
	return Field(nCol).AsFloat();
}

inline double CODBCRecordset::GetDouble(int nCol) const
{
	return Field(nCol).AsDouble();
}

inline COleDateTime CODBCRecordset::GetDate(int nCol) const
{
	return Field(nCol).AsDate();
}

inline CString CODBCRecordset::GetString(int nCol) const
{
	return Field(nCol).AsString();
}

inline CLongBinary* CODBCRecordset::GetBinary(int nCol) const
{
