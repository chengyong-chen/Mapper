...
CODBCRecordset::CODBCRecordset(CDatabase* pDatabase /*= nullptr*/)
...
BOOL CODBCRecordset::Open(LPCTSTR lpszSQL, UINT nOpenType, /*=AFX_DB_USE_DEFAULT_TYPE*/
						DWORD dwOptions /*= 0*/)
{
...
void CODBCRecordset::LoadFieldNamesMap()
{
	...
	const int nFields = m_nFields = GetODBCFieldCount();
	...
	for(int i = 0; i<nFields; i++)
	{
		...
		{
			fldCount++;
			cName.Format(_T("%s%d"), fi.m_strName, fldCount);
		}
...
int CODBCRecordset::GetFieldId(LPCTSTR szName) const
{
	void* idx = (void*)-1;
...
CString CODBCRecordset::GetFieldName(int nField) const
{
...
void CODBCRecordset::DoFieldExchange(CFieldExchange* pFX)
{
	...
	{
		cFieldName.Format(_T("[%s]"), GetFieldName(i));
...
{
	const int nFieldType = GetCFieldType(fi.m_nSQLType);
	switch(nFieldType)
	{
		case SQL_C_BIT:
...
void BinaryToString(CString& cStr, const CLongBinary& bin, bool bExpand)
{
	...
	TCHAR* pStr = cStr.GetBuffer(nSize);
	memcpy(pStr, p, nSize);
...
bool CDBField::AsBool() const
{
	...
	switch(m_dwType)
	{
		...
		case DBVT_STRING: ASSERT(m_pstring!=nullptr);
...
COleDateTime CDBField::AsDate() const
{
	...
	switch(m_dwType)
	{
		...
		case DBVT_DATE: ASSERT(m_pdate!=nullptr);
			return COleDateTime(m_pdate->year, m_pdate->month, m_pdate->day, m_pdate->hour, m_pdate->minute, m_pdate->second);
...
CString CDBField::AsString() const
{
	...
	switch(m_dwType)
	{
		...
		case DBVT_UCHAR:
			return CString((char)m_chVal);

		case DBVT_SHORT:
			cValue.Format(_T("%hd"), m_iVal);
			return cValue;

		case DBVT_LONG:
			cValue.Format(_T("%ld"), m_lVal);
			return cValue;

		case DBVT_SINGLE:
			cValue.Format(_T("%f"), m_fltVal);
			return cValue;

		case DBVT_DOUBLE:
			cValue.Format(_T("%f"), m_dblVal);
			return cValue;

		case DBVT_DATE:
			{
				ASSERT(m_pdate!=nullptr);
				const COleDateTime date(m_pdate->year, m_pdate->month, m_pdate->day, m_pdate->hour, m_pdate->minute, m_pdate->second);
				return date.Format();
			}
		case DBVT_STRING: ASSERT(m_pstring!=nullptr);
			return *m_pstring;

		case DBVT_BINARY: ASSERT(m_pbinary!=nullptr);
...
CDBField& CDBField::operator =(const bool bVal)
{
	switch(m_dwType)
	{
		...
		case DBVT_STRING: ASSERT(m_pstring!=nullptr);
			m_pstring->Format(_T("%c"), bVal ? 'T' : 'F');
			return *this;
...
CDBField& CDBField::operator =(const LPCTSTR szVal)
{
	switch(m_dwType)
	{
		...
		case DBVT_BOOL:
			m_boolVal = szVal!=nullptr||_ttoi(szVal)!=0;
			return *this;
		...
		case DBVT_SHORT:
			m_iVal = (short)_ttoi(szVal);
			return *this;

		case DBVT_LONG:
			m_lVal = (long)_ttol(szVal);
			return *this;

		case DBVT_SINGLE:
			m_fltVal = (float)_ttof(szVal);
			return *this;

		case DBVT_DOUBLE:
			m_dblVal = (double)_ttof(szVal);
			return *this;
