...
BOOL CRecordsetEx::Open(UINT nOpenType, LPCTSTR lpszSQL, DWORD dwOptions)
{
	ASSERT(!IsOpen());
	ASSERT_VALID(this);
	ASSERT(lpszSQL==nullptr||AfxIsValidString(lpszSQL));
...
void CRecordsetEx::AllocAndCacheFieldInfo()
{
	...
	for(WORD n = 1; n<=GetODBCFieldCount(); n++)
	{
		...
		AFX_ODBC_CALL(::SQLDescribeColA(m_hstmt, n,
...
#ifndef _UNICODE
		m_rgODBCFieldInfos[n-1].m_strName.ReleaseBuffer();
...
void CRecordsetEx::GetFieldValue(short nIndex, CString& strValue)
{
	...
	const int nLen = GetTextLen(m_rgODBCFieldInfos[nIndex-1].m_nSQLType,
	...
	const long nActualSize = GetData(m_pDatabase, m_hstmt, nIndex,
		SQL_C_CHAR, pvData, nLen,
		m_rgODBCFieldInfos[nIndex-1].m_nSQLType);

	// Handle nullptr data separately
