...
/////////////////////////////////////////////////////////////////////////////
// CLineAloneDlg dialog

extern BYTE d_nUnitIndex;
extern UNIT g_UnitArray[3];

CLineAloneDlg::CLineAloneDlg(CWnd* pParent, CLine* pline)
	: CDialog(CLineAloneDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CLineAloneDlg)
	d_miterlimit = 0;
	d_fWidth = 0.0f;
	//}}AFX_DATA_INIT
	d_pLine = pline;
	d_fWidth = (float)(d_pLine->m_nWidth/(g_UnitArray[d_nUnitIndex].pointpro*10));
}

void CLineAloneDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CLineAloneDlg)
	DDX_Text(pDX, IDC_MITERLIMIT, d_miterlimit);
	DDV_MinMaxInt(pDX, d_miterlimit, 0, 100);
	DDX_Text(pDX, IDC_WIDTH, d_fWidth);
	DDV_MinMaxFloat(pDX, d_fWidth, 0.0f, 1000.f);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CLineAloneDlg, CDialog)
	//{{AFX_MSG_MAP(CLineAloneDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CLineAloneDlg message handlers

BOOL CLineAloneDlg::OnInitDialog()
{
...
void CLineAloneDlg::OnOK()
{
	CDialog::OnOK();

	d_pLine->m_nWidth = (unsigned short)(round(d_fWidth*10*g_UnitArray[d_nUnitIndex].pointpro));
...
void CLineAloneDlg::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
