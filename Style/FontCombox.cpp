...
template<>
UINT AFXAPI HashKey(const CStringA& key)
{
	UINT nHash = 0;
	for(int index = 0; index < key.GetLength(); index++)
	{
		char c = key.GetAt(index);
...
CMap<CStringA, const CStringA&, CString, const CString&> CFontDesc::m_RealToFace;
CMap<CStringA, const CStringA&, CString, const CString&> CFontDesc::m_RealToFamily;

CFontDesc::CFontDesc()
{
	m_strFamily = _T("Arial");
	m_strFaceName = _T("Arial");
...
CFontDesc::CFontDesc(LPCTSTR lpszFaceName)
{
	...
	m_strFamily = CFontDesc::GetFamilyByReal(m_strRealName);

	m_style = Gdiplus::FontStyle::FontStyleRegular;
}
void CFontDesc::Sha1(boost::uuids::detail::sha1& sha1) const
{
	cstr::sha1(m_strFaceName, sha1);
	cstr::sha1(m_strRealName, sha1);
	cstr::sha1(m_strFamily, sha1);
...
const CFontDesc& CFontDesc::operator =(const CFontDesc& descSrc)
{
	if(&descSrc != this)
	{
		m_strFaceName = descSrc.m_strFaceName;
...
{
	if(m_strFamily != descSrc.m_strFamily)
		return FALSE;
	else if(m_strFaceName != descSrc.m_strFaceName)
		return FALSE;
...
void CFontDesc::SetFaceName(CString strFaceName)
{
...
void CFontDesc::SetRealName(CStringA strRealName)
{
	...
	m_strFaceName = CFontDesc::GetFaceByReal(strRealName);
	m_strFamily = CFontDesc::GetFamilyByReal(m_strRealName);
}
	...
	else
	{
		...
		if(FontRealNameExists(m_strRealName) == FALSE)
		{
			...
			if(fontFamily.IsAvailable() == TRUE)
			{
				...
				m_strFamily = family;
				m_strRealName = CFontDesc::GetRealByFamily(m_strFamily);
			}
			else
			{
				m_strFamily = m_strRealName;
			}
		}
		else
		{
		...
		m_strFaceName = CFontDesc::GetFaceByReal(m_strRealName);
...
DWORD CFontDesc::PackPC(CFile* pFile, BYTE*& bytes)
{
	...
	{
		m_strFaceName = UnpackStrPC(bytes);
...
void CFontDesc::LoadFontNames()
{
	...
	while(GetNextNameValue(HKEY_LOCAL_MACHINE, strKey, szName, szData) == ERROR_SUCCESS)
	{
		...
		if(strType == "TTF")
		{
			...
			{
				m_RealToFace.SetAt(fontprops.realName, fontprops.faceName);
				m_RealToFamily.SetAt(fontprops.realName, fontprops.csFamily);
...
CStringA CFontDesc::GetRealByFamily(CString strFamily)
{
...
char* CFontDesc::GetBestMatchedByFace(const char* family, const char* name)
{
	char name1[256] = {0};
	char name2[256] = {0};
