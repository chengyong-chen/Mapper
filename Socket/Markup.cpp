...
bool CMarkup::SetDoc(LPCTSTR szDoc)
{
	...
	const int nStartSize = m_csDoc.GetLength()/64+8;
...
bool CMarkup::Load(LPCTSTR szFileName)
{
	...
	try
	{
		...
		const int nWideLength = MultiByteToWideChar(CP_UTF8, 0, (const char*)pBuffer, nLength, nullptr, 0);
...
bool CMarkup::Save(LPCTSTR szFileName) const
{
	try
	{
...
#if defined(_UNICODE)
		const int nUTF8Len = WideCharToMultiByte(CP_UTF8, 0, m_csDoc, nLength, nullptr, 0, nullptr, nullptr);
...
bool CMarkup::FindElem(LPCTSTR szName)
{
	...
	if(m_aPos.GetSize())
	{
		const int iPos = x_FindElem(m_aPos[m_iPos].iElemParent, m_iPos, szName);
		if(iPos)
		{
			// Assign new position
			m_iPos = iPos;
...
bool CMarkup::FindChildElem(LPCTSTR szName)
{
	...
	if(!m_iPos)
		FindElem();
	const int iPosChild = x_FindElem(m_iPos, m_iPosChild, szName);
	if(iPosChild)
	{
		// Assign new position
...
CString CMarkup::GetPosData(int nStart, int nEnd) const
{
...
bool CMarkup::AddElem(LPCTSTR szName, LPCTSTR szValue)
{
	...
	const int iPosParent = m_iPos ? m_aPos[m_iPos].iElemParent : 0;
	m_iPosChild = 0;
...
bool CMarkup::AddChildSubDoc(LPCTSTR szSubDoc)
{
	...
	const int iPosSubDoc = x_AddSubDoc(m_iPos, iPosBefore, szSubDoc);
	if(!iPosSubDoc)
		return false;
...
bool CMarkup::InsertChildSubDoc(LPCTSTR szSubDoc)
{
	...
	// Add after previous position
	const int iPosSubDoc = x_AddSubDoc(m_iPos, iPosPrev, szSubDoc);
	if(!iPosSubDoc)
		return false;
...
CString CMarkup::GetChildSubDoc() const
{
	if(m_iPosChild)
	{
		const int nL = m_aPos[m_iPosChild].nStartL;
		const int nR = m_aPos[m_iPosChild].nNext;
...
int CMarkup::x_ParseElem(int iPosParent)
{
	...
	while(csName.IsEmpty())
	{
		...
		if(x_FindToken(token))
		{
			...
			const TCHAR cFirstChar = m_csDoc[token.nL];
			if(cFirstChar==_T('?'))
...
bool CMarkup::x_FindChar(int& n, _TCHAR c) const
{
	...
	const LPCTSTR szDoc = (LPCTSTR)m_csDoc;
...
int CMarkup::x_SetAttrib(int iPos, LPCTSTR szAttrib, LPCTSTR szValue)
{
	...
	if(iPos)
	{
		...
		else
		{
			...
			const CString csValue = x_TextToDoc(szValue, true);
		...
		const int nAdjust = csInsert.GetLength()-nReplace;
...
bool CMarkup::x_SetData(int iPos, LPCTSTR szData, int nCDATA)
{
	...
	if(ep.IsEmptyElement())
	{
		...
		const CString csTagName = x_GetTagName(iPos);
		ep.nStartR -= 1;
...
CString CMarkup::x_GetData(int iPos) const
{
	...
	if(!m_aPos[iPos].iElemChild&&!m_aPos[iPos].IsEmptyElement())
	{
		...
		{
			const int nEndCDATA = m_csDoc.Find(_T("]]>"), token.nNext);
...
CString CMarkup::x_TextToDoc(LPCTSTR szText, bool bAttrib) const
{
	...
	// &quot; double quote
	//
	static const _TCHAR* szaReplace[] = {_T("&lt;"), _T("&amp;"), _T("&gt;"), _T("&apos;"), _T("&quot;")};
	...
	while(cSource)
	{
		...
		if((pFound = (_TCHAR*)_tcschr(pFind, cSource))!=nullptr)
		{
...
CString CMarkup::x_TextFromDoc(int nLeft, int nRight) const
{
	...
	//
	static const _TCHAR* szaCode[] = {_T("lt;"),_T("amp;"),_T("gt;"),_T("apos;"),_T("quot;")};
	static int anCodeLen[] = {3,4,3,5,5};
	static const _TCHAR* szSymbol = _T("<&>\'\"");
	CString csText;
	const _TCHAR* pSource = m_csDoc;
	const int nDestSize = nRight-nLeft+1;
	_TCHAR* pDest = csText.GetBuffer(nDestSize);
	int nLen = 0;
	int nCharLen;
	int nChar = nLeft;
...
void CMarkup::x_DocChange(int nLeft, int nReplace, const CString& csInsert)
{
	...
	//
	const int nDocLength = m_csDoc.GetLength();
	const int nInsLength = csInsert.GetLength();
	...
	const int nNewLength = nInsLength+nDocLength-nReplace;
	const int nBufferLen = nNewLength;
...
int CMarkup::x_Add(int iPosParent, int iPosBefore, LPCTSTR szName, LPCTSTR szValue)
{
	...
	CString csInsert;
	const int nLenName = _tcslen(szName);
	...
	else
	{
		...
		const CString csValue = szValue; // x_TextToDoc(szValue);
		// </zhaowc>
	...
	if(bEmptyParent)
	{
		const CString csParentTagName = x_GetTagName(iPosParent);
...
int CMarkup::x_AddSubDoc(int iPosParent, int iPosBefore, LPCTSTR szSubDoc)
{
	...
	// if iPosBefore is nullptr, insert as first element under parent
	//
	const int nParentEndLBeforeAdd = m_aPos[iPosParent].nEndL;
	const int iPosFreeBeforeAdd = m_iPosFree;
...
#define SUBDOCFIND(c) {while(*szSD && *szSD!=c) szSD+=_tclen(szSD); if(*szSD!=c) return 0;}
	...
	if(iPos<=0)
	{
		...
		const CString csRevert = bEmptyParent ? _T("/") : _T("");
	...
	else
	{
		...
		const int iElemChild = m_aPos[iPos].iElemChild;
		m_aPos[iPos].iElemChild = 0;
		x_Adjust(iPos, csInsert.GetLength()-nReplace);
...
int CMarkup::x_AddAttrib(int iPos, LPCTSTR szAttrib, LPCTSTR szValue)
{
	...
	if(iPos)
	{
		...
		const CString csValue = x_TextToDoc(szValue, true);
		csInsert.Format(_T(" %s=\"%s\""), szAttrib, csValue);
		const int nL = m_aPos[iPos].nStartR-(m_aPos[iPos].IsEmptyElement() ? 1 : 0);
		x_DocChange(nL, 0, csInsert);
		const int nLen = csInsert.GetLength();
...
int CMarkup::x_Remove(int iPos)
{
	...
	// Return new position
	//
	const int iPosParent = m_aPos[iPos].iElemParent;
	...
	const int nLen = m_aPos[iPos].nNext-m_aPos[iPos].nStartL;
