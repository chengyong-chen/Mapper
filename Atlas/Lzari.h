...
class __declspec(dllexport) CLzari 
{
public:	
	CLzari();
	virtual ~CLzari();
protected:
	...
	const unsigned char *m_pInBuffer;
	int m_nInLength;
	int m_nInCur;
...
public:
	unsigned char* Compress(const unsigned char *pInBuffer,int nInLength,int &nOutLength);
	unsigned char* UnCompress(const unsigned char *pInBuffer,int nInLength,int &nOutLength);
