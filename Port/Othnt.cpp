...
COthnt::COthnt(CDatainfo& datainfo,LPCTSTR lpszCoveragePath) 
	   :CCoverage(datainfo,lpszCoveragePath)
{ 
	m_strCoveragePath += _T("\\Othnt");
}
...
long COthnt::ReadPAT()	// 读取多边形与弧段数据
{
...
BOOL COthnt::Import(CLayerTree& layertree)
{
	CMapLayer layermap;
	
	CLayer* layer = CreateLayers(layertree,layermap,_T("Othnt"));
	if(layer == nullptr)
		return false;
/*	
