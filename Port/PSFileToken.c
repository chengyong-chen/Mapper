...
int CPSFileToken:read_newline(int c)
{
    if (c == '\r')
    {
		c = GETC();
 ...
 */
static PyObject* read_dsc_comment()
{
    int size;
    int maxsize = 256;
    PyObject * value;
 ...
 * input.
 */
static PyObject * read_comment(PSTokenizerObject * self)
{
    int c;
    PyObject * value = NULL;
 ...
 */
static PyObject * read_string(PSTokenizerObject * self)
{
    int depth = 0;
    int size;
    int maxsize = 256;
 ...
 *
 */
static PyObject * read_name_or_number(PSTokenizerObject * self, int * token, int isname)
{
    int size;
    int maxsize = 256;
    PyObject * value;
	...
	*buf++ = c;
	...
	if (char_types[(int)*p] & FLOATCHAR)
	{
	    ...
	    old_locale = _strdup(setlocale(LC_NUMERIC, NULL));
