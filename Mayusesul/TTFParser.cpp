...
bool TTFParser::Parse(const std::wstring& szFilePath)
{
	Clear();
...
bool TTFParser::Parse(FILE* pFile, size_t nOffsetFromStart)
{
	Clear();
	if(nullptr==pFile)
		return false;
...
bool TTFParser::ParseOffsetTable()
{
...
bool TTFParser::Parse(BYTE* pFile, size_t size)
{
	return Parse(pFile, size, 0);
}

bool TTFParser::Parse(BYTE* pFile, size_t size, size_t offset)
{
	Clear();
	if(pFile==nullptr)
		return false;
...
bool TTFParser::ParseOffsetTableMem()
{
	stOffsetTable OffsetTable;
	memset(&OffsetTable, 0, sizeof OffsetTable);
...
bool TTFParser::ParseName(stTableDirectory& TableDirectory)
{
...
bool TTFParser::ParseNameMem(stTableDirectory& TableDirectory)
{
	EndianSwap::Switch(TableDirectory.offset);
...
std::wstring TTFParser::GetNameID(USHORT uNameID)
{
	switch(uNameID)
	{
		case 0:
...
void TTFParser::SetNameIDValue(USHORT uNameID, const std::wstring& str)
{
	switch(uNameID)
	{
		case 0:
...
void TTFParser::SetNameIDValue(USHORT uNameID, const std::string& str)
{
	SetNameIDValue(uNameID, NarrowToWide(str));
}

std::wstring TTFParser::NarrowToWide(const std::string& szNarrow)
{
	int size = MultiByteToWideChar(CP_ACP,           // code page
									0,    // character-type options
									szNarrow.c_str(), // string to map
									szNarrow.size(),  // number of bytes in string
									nullptr,  // wide-character buffer
									0      // size of buffer
	);
	size++;
	WCHAR* pBuf = new WCHAR[size];
	memset(pBuf, 0, size*sizeof(WCHAR));
	int size2 = MultiByteToWideChar(CP_ACP,           // code page
									0,    // character-type options
									szNarrow.c_str(), // string to map
									szNarrow.size(),  // number of bytes in string
...
std::string TTFParser::WideToNarrow(const std::wstring& szWide)
{
	BOOL bUsedDefaultChar = FALSE;
	int size = WideCharToMultiByte(CP_ACP,           // code page
									0,    // character-type options
									szWide.c_str(), // string to map
									szWide.size(),  // number of bytes in string
									nullptr,  // character buffer
									0,      // size of buffer
	...
	int size2 = WideCharToMultiByte(CP_ACP,           // code page
									0,    // character-type options
									szWide.c_str(), // string to map
									szWide.size(),  // number of bytes in string
...
bool TTFParser::IsBold() const
{
	return m_bBold;
}

bool TTFParser::IsItalic() const
{
	return m_bItalic;
}

bool TTFParser::IsRegular() const
{
	return m_bRegular;
}

std::wstring TTFParser::ToLower(const std::wstring& szNormal)
{
...
std::string TTFParser::ToLower(const std::string& szNormal)
{
	std::string szLower = "";
	for(size_t i = 0; i<szNormal.size(); ++i)
	{
