...
CGridCtrl::~CGridCtrl()
{
	...
	{
		m_TitleTip.PostMessage(WM_DESTROY, 0, 0);
...
{ //  gives back the size of the area (do not forget to delete pLayer)
	const int Length = 2+GetColumnCount()*2;
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	{
		const CCellID idOldTopLeft = GetTopleftNonFixedCell();
		SendMessage(WM_VSCROLL, SB_PAGEDOWN, 0);
		bVertScrollAction = TRUE;
		const CCellID idNewTopLeft = GetTopleftNonFixedCell();
...
CCellID CGridCtrl::SetFocusCell(CCellID cell)
{
	...
	const CCellID idPrev = m_idCurrentCell;
	...
	BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
	...
	const int Left = (m_AllowSelectRowInFixedCol ? 0 : GetFixedColumnCount());
...
{
	USES_CONVERSION;
	const CCellRange Selection = GetSelectedCellRange();
	...
	if(GetVirtualMode())
		SendCacheHintToParent(CCellRange(-1, -1, -1, -1));
	const DWORD dwLen = (DWORD)sf.GetLength();
	...
	BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	// to hold this.
	const LPSTR szBuffer = new char[::GlobalSize(hmem)]; // FIX: Use LPSTR char here
	if(!szBuffer)
		return FALSE;
	...
	do
	{
		...
		const CString strNext = (nIndex<0) ? CString(_T("")) : strLine.Mid(nIndex+1);
...
void CGridCtrl::OnBeginDrag()
{
	...
	if(pSource)
	{
		...
		m_bLMouseButtonDown = FALSE;
		const DROPEFFECT dropEffect = pSource->DoDragDrop(DROPEFFECT_COPY|DROPEFFECT_MOVE);
	...
	CPoint /* point */)
{
	...
	if(m_CurCol>=0)
	{
		...
		{
			const int New = m_arColOrder[m_CurCol];
...
BOOL CGridCtrl::MouseOverRowResizeArea(const CPoint& point)
{
	if(point.x>=GetFixedColumnWidth())
		return FALSE;
	const CCellID idCurrentCell = GetCellFromPt(point);
	CPoint start;
	...
	//by Huang Wei
	const int endy = start.y+GetMergeCellHeight(idCurrentCell);
...
BOOL CGridCtrl::MouseOverColumnResizeArea(const CPoint& point)
{
	...
	//Merge the selected cells 
	//by Huang Wei
	const int endx = start.x+GetMergeCellWidth(idCurrentCell);
...
CCellID CGridCtrl::GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck /*=TRUE*/)
{
	...
	const CCellID idTopLeft = GetTopleftNonFixedCell();
	...
	BOOL bForceRecalculation /*=FALSE*/)
{
	...
	CRect rect;
	GetClientRect(rect);
	const CCellID idTopLeft = GetTopleftNonFixedCell(bForceRecalculation);
	...
	if(nHitTest==HTCLIENT)
	{
		...
		default:
			if(!GetVirtualMode())
			{
				...
				ScreenToClient(&pt);
				const CCellID cell = GetCellFromPt(pt);
				if(IsValid(cell))
				{
					CGridCellBase* pCell = GetCell(cell.row, cell.col);
					if(pCell)
	...
	int low, int high)
{
	...
	const LPARAM pMidCell = (LPARAM)GetCell((lo+hi)/2, nCol);
...
BOOL CGridCtrl::GetItem(GV_ITEM* pItem) const
{
	if(!pItem)
		return FALSE;
	CGridCellBase* pCell = GetCell(pItem->row, pItem->col);
...
BOOL CGridCtrl::SetItemFormat(int nRow, int nCol, UINT nFormat) const
{
	if(GetVirtualMode())
		return FALSE;
...
BOOL CGridCtrl::SetItemBkColour(int nRow, int nCol, COLORREF cr /* = CLR_DEFAULT */) const
{
	if(GetVirtualMode())
		return FALSE;
...
BOOL CGridCtrl::SetItemFgColour(int nRow, int nCol, COLORREF cr /* = CLR_DEFAULT */) const
{
	if(GetVirtualMode())
		return FALSE;
...
BOOL CGridCtrl::SetItemFont(int nRow, int nCol, const LOGFONT* plf) const
{
	if(GetVirtualMode())
		return FALSE;
...
const LOGFONT* CGridCtrl::GetItemFont(int nRow, int nCol) const
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if(!pCell)
...
BOOL CGridCtrl::IsItemEditing(int nRow, int nCol) const
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if(!pCell)
...
void CGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
	...
	const int nDifference = rect.Width()-(int)virtualWidth;
...
void CGridCtrl::ExpandRowsToFit(BOOL bExpandFixed /*=TRUE*/)
{
	...
	const int nDifference = rect.Height()-(int)virtualHeight;
...
BOOL CGridCtrl::IsCellFixed(int nRow, int nCol) const
{
...
BOOL CGridCtrl::GetModified(int nRow /*=-1*/, int nCol /*=-1*/) const
{
	if(nRow>=0&&nCol>=0)
...
void CGridCtrl::EnsureVisible(int nRow, int nCol)
{
	...
	*/
	...
	const CCellRange VisibleCells = GetVisibleNonFixedCellRange();
...
BOOL CGridCtrl::InvalidateCellRect(const CCellRange& cellRange)
{
	...
	if(!::IsWindow(GetSafeHwnd())||!m_bAllowDraw)
		return FALSE;
	const CCellRange visibleCellRange = GetVisibleNonFixedCellRange().Intersect(cellRange);
...
BOOL CGridCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	{
		const int nRowsScrolled = m_nRowsPerWheelNotch*zDelta/120;
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if(m_MouseMode==MOUSE_OVER_COL_DIVIDE) // sizing column
	{
		...
		if(!m_bHiddenColUnhide)
		{
			...
			const BOOL bIsCellRightBorder = point.x-start.x>=m_nResizeCaptureRange;
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	else if(m_MouseMode==MOUSE_SIZING_COL)
	{
		...
		if(m_LeftClickDownPoint!=point&&(point.x!=0||point.y!=0)) // 0 pt fix by email1@bierling.net
		{
			...
			if(!GetCellOrigin(m_LeftClickDownCell, &start))
				return;
			const int nColumnWidth = __max(point.x-start.x, m_bAllowColHide ? 0 : 1);
			//Used for merge cells
			//by Huang Wei
			const int mergewidth = GetMergeCellWidth(m_LeftClickDownCell)-GetColumnWidth(m_LeftClickDownCell.col);
	...
	else if(m_MouseMode==MOUSE_SIZING_ROW)
	{
		...
		if(m_LeftClickDownPoint!=point&&(point.x!=0||point.y!=0)) // 0 pt fix by email1@bierling.net
		{
			...
			//Used for merge cells
			//by Huang Wei
			const int mergeheight = GetMergeCellHeight(m_LeftClickDownCell)-GetRowHeight(m_LeftClickDownCell.row);
...
void CGridCtrl::GetPrintMarginInfo(int& nHeaderHeight, int& nFooterHeight,
	int& nLeftMargin, int& nRightMargin, int& nTopMargin,
	int& nBottomMargin, int& nGap) const
{
...
void CGridCtrl::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo)
{
	...
	m_CharSize.cx /= 52;
	const int nMargins = (m_nLeftMargin+m_nRightMargin)*m_CharSize.cx;
...
BOOL CGridCtrl::Save(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/) const
{
	CStdioFile File;
	CFileException ex;
...
CImageList* CGridCtrl::CreateDragImage(CPoint* pHotSpot)
{
	...
	CRect rect;
	const CCellID cell = GetFocusCell();
...
CSize CGridCtrl::GetTextExtent(int nRow, int nCol, LPCTSTR str) const
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	if(!pCell)
		return CSize(0, 0);
...
//Merge the selected cells 
//by Huang Wei
CGridCellBase* CGridCtrl::GetCell(CCellID cell) const
{
...
{
	const CCellRange range = GetSelectedCellRange();
...
void CGridCtrl::MergeCells(int nStartRow, int nStartCol, int nEndRow, int nEndCol)
{
	for(int row = nStartRow; row<=nEndRow; row++)
	{
		for(int col = nStartCol; col<=nEndCol; col++)
		{
			...
			{
				const CCellRange range(nStartRow, nStartCol, nEndRow, nEndCol);
				pCell->SetMergeRange(range);
			}
			else
			{
				const CCellID cell(nStartRow, nStartCol);
...
{
	const CCellID mergecell = GetMergeCellID(cell);
