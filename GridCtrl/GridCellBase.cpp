...
// C Maunder     - 29 Aug 2000 - operator= checks for nullptr font before setting (Martin Richter)
...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	{
		const CCellID FocusCell = pGrid->GetFocusCell();
...
CSize CGridCellBase::GetTextExtent(LPCTSTR szText, CDC* pDC /*= nullptr*/)
{
	...
	if((nFormat&DT_WORDBREAK)&&!(nFormat&DT_SINGLELINE))
	{
		...
		while(TRUE)
		{
			...
			const int nTempWidth = pDC->GetTextExtent(TempStr).cx;
...
*****************************************************************************/
LRESULT CGridCellBase::SendMessageToParent(int nRow, int nCol, int nMessage) const
{
	CGridCtrl* pGrid = GetGrid();
	if(pGrid)
