...
class __declspec(dllexport) CGridCtrl : public CWnd
{
...
public:
	...
	CSize GetTextExtent(int nRow, int nCol, LPCTSTR str) const;
	// EFW - Get extent of current text in cell
	CSize GetCellTextExtent(int nRow, int nCol) const
	{
	...
	GRIDCALLBACK GetCallbackFunc() const
	{
...
public:
	...
	BOOL GetModified(int nRow = -1, int nCol = -1) const;
	BOOL IsCellFixed(int nRow, int nCol) const;

	BOOL SetItem(const GV_ITEM* pItem);
	BOOL GetItem(GV_ITEM* pItem) const;
	...
	BOOL SetItemFormat(int nRow, int nCol, UINT nFormat) const;
	UINT GetItemFormat(int nRow, int nCol) const;
	BOOL SetItemBkColour(int nRow, int nCol, COLORREF cr = CLR_DEFAULT) const;
	COLORREF GetItemBkColour(int nRow, int nCol) const;
	BOOL SetItemFgColour(int nRow, int nCol, COLORREF cr = CLR_DEFAULT) const;
	COLORREF GetItemFgColour(int nRow, int nCol) const;
	BOOL SetItemFont(int nRow, int nCol, const LOGFONT* lf) const;
	const LOGFONT* GetItemFont(int nRow, int nCol) const;

	BOOL IsItemEditing(int nRow, int nCol) const;
...
#ifndef _WIN32_WCE
	BOOL Save(LPCTSTR filename, TCHAR chSeparator = _T(',')) const;
...
public:
	...
	//by Huang Wei
	CGridCellBase* GetCell(CCellID cell) const;
...
protected:
	...
	{
		const int nOldMode = m_MouseMode;
		m_MouseMode = nMode;
		return nOldMode;
	}
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const
{
	...
	if(GetVirtualMode())
	{
		const CGridCellBase* pCell = GetDefaultCell(nRow<m_nFixedRows, nCol<m_nFixedCols);
