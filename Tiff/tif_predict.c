...
PredictorSetupDecode(TIFF* tif)
{
	...
	if(sp->predictor==2)
	{
		...
		{
		case 8: sp->pfunc = horAcc8;
			break;
		case 16: sp->pfunc = horAcc16;
			break;
		}
		/*
...
PredictorSetupEncode(TIFF* tif)
{
	...
	if(sp->predictor==2)
	{
		switch(td->td_bitspersample)
		{
		case 8: sp->pfunc = horDiff8;
			break;
		case 16: sp->pfunc = horDiff16;
			break;
		}
		/*
...
{
	const TIFFPredictorState* sp = PredictorState(tif);
	const u_int stride = sp->stride;
...
{
	const u_int stride = PredictorState(tif)->stride;
	uint16* wp = (uint16*)cp0;
	tsize_t wc = cc/2;
...
PredictorDecodeTile(TIFF* tif, tidata_t op0, tsize_t occ0, tsample_t s)
{
	...
	{
		const tsize_t rowsize = sp->rowsize;
		assert(rowsize>0);
		assert(sp->pfunc!=NULL);
		while((long)occ0>0)
		{
...
PredictorPrintDir(TIFF* tif, FILE* fd, long flags)
{
	...
	if(TIFFFieldSet(tif, FIELD_PREDICTOR))
	{
		...
		switch(sp->predictor)
		{
		case 1: fprintf(fd, "none ");
			break;
		case 2: fprintf(fd, "horizontal differencing ");
			break;
		}
