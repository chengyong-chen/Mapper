...
{
	TIFF* tif = img->tif;
	const tileContigRoutine put = img->put.contig;
	uint16 orientation;
	uint32 col, row, y;
	uint32 tw, th;
	u_char* buf;
...
{
	TIFF* tif = img->tif;
	const tileSeparateRoutine put = img->put.separate;
	uint16 orientation;
	uint32 col, row, y;
	uint32 tw, th;
	u_char* buf;
...
gtStripContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
	...
	u_char* buf;
	uint32 rowsperstrip;
	const uint32 imagewidth = img->width;
	tsize_t scanline;
	int32 fromskew, toskew;
...
{
	const int samplesperpixel = img->samplesperpixel;
...
{
	const TIFFRGBValue* Map = img->Map;
	const int samplesperpixel = img->samplesperpixel;
...
TIFFYCbCrToRGBInit(TIFFYCbCrToRGB* ycbcr, TIFF* tif)
{
	...
	{
		const float f1 = 2-2*LumaRed;
		const int32 D1 = FIX(f1);
		const float f2 = LumaRed*f1/LumaGreen;
		const int32 D2 = -FIX(f2);
		const float f3 = 2-2*LumaBlue;
		const int32 D3 = FIX(f3);
		const float f4 = LumaBlue*f3/LumaGreen;
		const int32 D4 = -FIX(f4);
		int x;
...
{
	const TIFFRGBValue* Map = img->Map;
	const int bitspersample = img->bitspersample;
	const int nsamples = 8/bitspersample;
	int i;
	uint32* p;
...
pickTileContigCase(TIFFRGBAImage* img)
{
	...
	if(buildMap(img))
	{
		...
		case PHOTOMETRIC_PALETTE:
			switch(img->bitspersample)
			{
			case 8: put = put8bitcmaptile;
				break;
			case 4: put = put4bitcmaptile;
				break;
			case 2: put = put2bitcmaptile;
				break;
			case 1: put = put1bitcmaptile;
				break;
			}
			break;
		case PHOTOMETRIC_MINISWHITE:
		case PHOTOMETRIC_MINISBLACK:
			switch(img->bitspersample)
			{
			case 8: put = putgreytile;
				break;
			case 4: put = put4bitbwtile;
				break;
			case 2: put = put2bitbwtile;
				break;
			case 1: put = put1bitbwtile;
				break;
			}
			break;
		case PHOTOMETRIC_YCBCR:
