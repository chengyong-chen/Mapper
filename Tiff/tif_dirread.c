...
TIFFReadDirectory(TIFF* tif)
{
	...
	for(dp = dir, n = dircount; n>0; n--, dp++)
	{
		...
		{
			const uint32 expected = (fip->field_readcount==TIFF_SPP) ?
...
EstimateStripByteCounts(TIFF* tif, TIFFDirEntry* dir, uint16 dircount)
{
	...
	if(td->td_compression!=COMPRESSION_NONE)
	{
		uint32 space = (uint32)(sizeof(TIFFHeader)
			...
			+(dircount*sizeof(TIFFDirEntry))
			+sizeof(uint32));
		const toff_t filesize = TIFFGetFileSize(tif);
		uint16 n;

		/* calculate amount of space used by indirect values */
		for(dp = dir, n = dircount; n>0; n--, dp++)
		{
			const uint32 cc = dp->tdir_count*tiffDataWidth[dp->tdir_type];
			if(cc>sizeof(uint32))
				space += cc;
		}
	...
	{
		const uint32 rowbytes = TIFFScanlineSize(tif);
		const uint32 rowsperstrip = td->td_imagelength/td->td_stripsperimage;
		for(i = 0; i<td->td_nstrips; i++)
...
{
	const int w = tiffDataWidth[dir->tdir_type];
	const tsize_t cc = dir->tdir_count*w;
...
TIFFFetchAnyArray(TIFF* tif, TIFFDirEntry* dir, double* v)
{
	...
	case TIFF_SBYTE:
		...
		{
			const uint16* vp = (uint16*)v;
			for(i = dir->tdir_count-1; i>=0; i--)
				v[i] = vp[i];
		}
		else
		{
			const int16* vp = (int16*)v;
			for(i = dir->tdir_count-1; i>=0; i--)
				v[i] = vp[i];
		}
		break;
	...
	case TIFF_SLONG:
		...
		{
			const uint32* vp = (uint32*)v;
			for(i = dir->tdir_count-1; i>=0; i--)
				v[i] = vp[i];
		}
		else
		{
			const int32* vp = (int32*)v;
			for(i = dir->tdir_count-1; i>=0; i--)
				v[i] = vp[i];
		}
		break;
...
TIFFFetchNormalTag(TIFF* tif, TIFFDirEntry* dp)
{
	...
	{ /* singleton value */
		...
		case TIFF_SSHORT:
			 ...
			 */
		{
			const TIFFDataType type = fip->field_type;
...
{
	const int samples = tif->tif_dir.td_samplesperpixel;
	int status = 0;
...
{
	const int samples = (int)tif->tif_dir.td_samplesperpixel;
	int status = 0;
