...
TIFFComputeTile(TIFF* tif, uint32 x, uint32 y, uint32 z, tsample_t s)
{
	...
	if(dx!=0&&dy!=0&&dz!=0)
	{
		const uint32 xpt = TIFFhowmany(td->td_imagewidth, dx);
		const uint32 ypt = TIFFhowmany(td->td_imagelength, dy);
		const uint32 zpt = TIFFhowmany(td->td_imagedepth, dz);
...
TIFFVTileSize(TIFF* tif, uint32 nrows)
{
		...
		!isUpSampled(tif))
	{
		...
		const tsize_t rowsize = TIFFhowmany(w*td->td_bitspersample, 8);
		const tsize_t samplingarea =
