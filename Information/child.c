   ...
   switch(lpChild->dwOperation) {
     case  OPER_SELECT:
      ...
      Async(SQLExecDirect(lpChild->hstmt,
                          ...
                          lstrlen(lpChild->sql)));
	  STMTError(SQLSetPos(lpChild->hstmt, 5, SQL_POSITION,SQL_LOCK_NO_CHANGE));
      break;
...
SQLRETURN INTFUNC PrepareStmt(LPCHILD lpChild)
{
   ...
   if (g_fAsyncSupported && STMTError(SQLSetStmtAttr(lpChild->hstmt,SQL_ATTR_ASYNC_ENABLE,(SQLPOINTER) (lpChild->fAsync ? 1 : 0),SQL_IS_INTEGER)))
      return SQL_ERROR;
...
void INTFUNC GetCurrentValue(LPTSTR lpsz, LPCOL lpcol, LPCHILD lpChild)
{
   LPBYTE      lpb;
   SDWORD FAR  *lpcb;
   UWORD       irowPos;
